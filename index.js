...
app.patch("/product/:id/upvotes",async (req,res)=>{
    try {
        ...
        const user=req.body;
        if(! ( validationFunction.validateId(prdid) && validationFunction.validateId(user._id.$oid)))
...
app.post("/product", async (req, res) => {
    try {
        ...
        res.status(err.status||400).send(`error code of mongoose is ${err.message.slice(0,5)} and error is ${err.message.slice(6)}`);
    }
});
...
app.delete("/product/:id",async (req,res)=>{
    try{
        ...
        res.status(400).send("Product don't exist");
    }
    catch(err){
        res.status(400).send(err.message)
    }
